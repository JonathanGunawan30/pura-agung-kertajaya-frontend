...
export function CalendarWidget({ activities, entityType = "pura" }: CalendarWidgetProps) {
  ...
  selectedActivities.sort((a, b) => a.time_info.localeCompare(b.time_info))
    ...
    <div className="flex flex-col lg:flex-row gap-8 bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 md:p-8">
      ...
      <div className="w-full lg:w-5/12 border-t lg:border-t-0 lg:border-l border-gray-100 dark:border-gray-800 pt-8 lg:pt-0 lg:pl-8">
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto max-h-[400px] pr-2 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-800">
            {selectedActivities.length > 0 ? (
              selectedActivities.map((act) => (
                <div 
                  key={act.id} 
                  className={`bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border-l-4 ${theme.cardBorder} animate-in fade-in slide-in-from-bottom-2 duration-300`}
                >
                  <h4 className="font-bold text-gray-900 dark:text-white mb-2 line-clamp-2 leading-snug">
                    {act.title}
                  </h4>
